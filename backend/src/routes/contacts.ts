import { Router } from 'express';
import { logTelemetryEvent } from '../config/db.js';
import { requireAdminAuth } from '../middleware/auth.js';
import { publicFormLimiter } from '../middleware/rateLimit.js';
import { AppError } from '../middleware/errorHandler.js';
import { contactCreateSchema, contactReplySchema } from '../schemas/forms.js';
import * as contactService from '../services/contactService.js';
import { notifyLead } from '../utils/notify.js';

const router = Router();

router.get('/', requireAdminAuth, async (_req, res, next) => {
    try {
        await logTelemetryEvent('contacts.list', 'ok');
        const data = await contactService.listContacts();
        res.json(data);
    } catch (e) {
        next(e);
    }
});

router.post('/', publicFormLimiter, async (req, res, next) => {
    try {
        const body = contactCreateSchema.parse(req.body);
        await logTelemetryEvent('contacts.create', 'ok');
        const row = await contactService.createContact(body);
        void notifyLead('New contact submission', { id: String(row.id) });
        res.json(row);
    } catch (e) {
        next(e);
    }
});

router.post('/:id/reply', requireAdminAuth, async (req, res, next) => {
    try {
        const { response_text } = contactReplySchema.parse(req.body);
        const { id } = req.params;
        await logTelemetryEvent('contacts.reply', 'ok');
        const row = await contactService.replyContact(id, response_text);
        if (!row) throw new AppError(404, 'Record not found', 'NOT_FOUND');
        res.json(row);
    } catch (e) {
        next(e);
    }
});

export default router;
